...
function renderHomepageBody() {
  ...
  <section class="perspective section" aria-labelledby="perspective-title" itemprop="publisher" itemscope itemtype="https://schema.org/Organization" itemid="${escapeHtml(SITE_IDENTITY.organizationId)}"><link itemprop="url" href="${escapeHtml(SITE_IDENTITY.siteUrl)}"><link itemprop="logo" href="${escapeHtml(SITE_IDENTITY.logoUrl)}"><div class="shell perspective-grid"><div class="perspective-image" data-reveal><img src="/casa2.jpg" width="640" height="853" alt="Vista exterior de una vivienda contemporánea" loading="lazy"></div><div class="perspective-copy" data-reveal><p class="eyebrow"><span></span> <span itemprop="name">${escapeHtml(SITE_IDENTITY.name)}</span></p><h2 id="perspective-title">Evaluación inmobiliaria con una mirada económica y constructiva.</h2><p class="large-copy" data-page-editorial itemprop="description">${escapeHtml(SITE_IDENTITY.description)}</p><p>Según el caso, la alternativa puede incluir inversión directa, mejoramiento o remodelación previa a la venta y otras vías de comercialización.</p><p>BRENA cuenta con experiencia directa en construcción, mejoramiento y remodelación. Cuando la evaluación lo requiere, puede coordinar una visita presencial a la propiedad.</p><p>BRENA recibe solicitudes desde distintas regiones de Chile. La viabilidad se confirma individualmente según la ubicación, las características de la propiedad y la alternativa requerida.</p><p>BRENA no garantiza compra, venta, precio, plazo ni valorización. Completar el formulario no obliga a continuar.</p></div></div></section>`;
}
