...
function renderFaq(page) {
  const faqs = page.faqs.length ? page.faqs : [
    { question: "¿BRENA compra todas las propiedades?", answer: "No. Cada caso se evalúa y la alternativa puede combinar inversión directa, mejoras o distintas vías de comercialización según su viabilidad." },
    { question: "¿Completar el formulario me obliga a vender?", answer: "No. El formulario permite una primera evaluación y contacto. Tú decides si quieres continuar." },
  ];
...
function renderContact(page) {
  return `<section class="contact-section section" id="conversemos" aria-labelledby="contact-title"><div class="shell contact-grid"><div class="contact-copy" data-reveal><p class="eyebrow eyebrow-light"><span></span> Hablemos de tu propiedad</p><h2 id="contact-title">Cuéntanos lo esencial. Nosotros hacemos las preguntas correctas.</h2><p>El formulario es el mismo para todas las situaciones. Elige expresamente la opción que corresponda a tu caso.</p><ul><li><span aria-hidden="true">✓</span> Información tratada de forma confidencial</li><li><span aria-hidden="true">✓</span> Sin documentos para comenzar</li><li><span aria-hidden="true">✓</span> Sin obligación de continuar</li></ul></div><div class="form-card" data-reveal><div class="form-topline"><span data-step-label>Paso 1 de 2</span><div class="progress-track" aria-hidden="true"><span data-progress></span></div></div><!-- lead-form:start -->${renderLeadForm({ relatedSituation: page.relatedSituation })}<!-- lead-form:end --></div></div></section>`;
}
...
<body>
  ...
  <footer class="site-footer" id="privacidad"><div class="shell footer-grid"><div><a class="brand brand-footer" href="/" aria-label="Brena, inicio"><img src="/brena.png" width="500" height="115" alt="Brena Gestión Inmobiliaria"></a><p>Evaluamos propiedades complejas para estructurar alternativas según cada caso.</p></div><div><strong>Privacidad</strong><p>Usamos los datos del formulario únicamente para evaluar tu solicitud y contactarte.</p></div></div><div class="shell footer-bottom"><span>© <span data-current-year>2026</span> Brena</span><a href="#inicio">Volver arriba ↑</a></div></footer>
</body>
</html>
`;
}
